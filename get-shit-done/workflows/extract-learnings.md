...
if [ -n "$CLAUDE_PLUGIN_ROOT" ] && \
  ...
  # Exit this step cleanly; the rest of the workflow is unaffected.
fi
```

If `INSTINCT_CLI` is unset after the block above, skip the remaining substeps.
...
1. Write a draft skill definition to `skills/{name}/SKILL.md` in the current repo (creating the directory if needed).
...
2. Note the file path and confirm to the user:
   ```
   Draft written: skills/{name}/SKILL.md
